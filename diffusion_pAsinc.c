...
double calculate_Tmean (float *grid, float *grid_chips, float *grid_aux, float t_delta, int max_iter, float t_ext, int NROW_loc, int NROW_glob, int NCOL_glob)
{
  ...
  while (end == 0)
  {
    ...
    if (pid==0){
      MPI_Isend(&grid[NROW_loc*NCOL_glob], NCOL_glob, MPI_FLOAT, pid+1, 0, MPI_COMM_WORLD, &req[nreq++]); //cada pid le envía su última fila(real) al siguiente pid
      MPI_Irecv(&grid[(NROW_loc+1)*NCOL_glob], NCOL_glob, MPI_FLOAT, pid+1, 0, MPI_COMM_WORLD, &req[nreq++]); //Todos los pid menos el 0 esperan a recibir la última fila del anterior pid para poder hacer los cálculos
    }
    else if(pid == npr-1){
      MPI_Irecv(&grid[0], NCOL_glob, MPI_FLOAT, pid-1, 0, MPI_COMM_WORLD,&req[nreq++]); //Todos los pid menos el 0 esperan a recibir la última fila del anterior pid para poder hacer los cálculos
      MPI_Isend(&grid[NCOL_glob], NCOL_glob, MPI_FLOAT, pid-1, 0, MPI_COMM_WORLD,&req[nreq++]); //cada pid le envía su última fila(real) al siguiente pid
    }
    else{
      MPI_Irecv(&grid[0], NCOL_glob, MPI_FLOAT, pid-1, 0, MPI_COMM_WORLD, &req[nreq++]); //Todos los pid menos el 0 esperan a recibir la última fila del anterior pid para poder hacer los cálculos
      MPI_Isend(&grid[NCOL_glob], NCOL_glob, MPI_FLOAT, pid-1, 0, MPI_COMM_WORLD, &req[nreq++]); //cada pid le envía su última fila(real) al siguiente pid
